...
### 受注直結の抜けKW（柱2・次点）
- [x] ホームページ リニューアル タイミング/費用/進め方 → ✅ 公開済み `homepage-renewal/`（7つのサイン・費用相場・進め方7ステップ・301リダイレクト/SEO引き継ぎ）
- [x] ホームページ 運用・保守とは（費用相場・自分でできること） → ✅ 公開済み `homepage-unyou-hoshu/`（運用/保守の違い・放置リスク・作業内容・月額/スポット費用相場・自分でできること/プロに任せること・保守契約の選び方）
- [x] ホームページ 制作 期間（最短5営業日が強み） → `homepage-seisaku-nagare/` 内で期間目安＋最短5営業日を解説
- [x] ホームページ 制作 流れ（依頼〜公開の全ステップ） → ✅ 公開済み `homepage-seisaku-nagare/`（10ステップ早見表・期間・準備物チェックリスト・進めるコツ）
